...
function AuthenticatedDashboard({ username, onSignOut }: { username: string; onSignOut: () => Promise<void> }) {
  ...
  const pauseMutation = trpc.bot.controls.setPaused.useMutation({ onSuccess: refreshSharedState });
  const watchlistMutation = trpc.bot.controls.setWatchlist.useMutation({ onSuccess: refreshSharedState });
  const timeframesMutation = trpc.bot.controls.setTimeframes.useMutation({ onSuccess: refreshSharedState });
  const thresholdMutation = trpc.bot.controls.setThreshold.useMutation({ onSuccess: refreshSharedState });
  const cooldownMutation = trpc.bot.controls.setCooldown.useMutation({ onSuccess: refreshSharedState });
  const rulesMutation = trpc.bot.controls.setRuleFamilies.useMutation({ onSuccess: refreshSharedState });
  ...
  const saveCooldown = () => {
    ...
    if (Number.isInteger(cooldownMinutes) && cooldownMinutes >= 1 && cooldownMinutes <= 1440) cooldownMutation.mutate({ cooldownMinutes });
  };
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]}><ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}><View style={styles.shell}>
    <View style={[styles.topbar, { borderColor: colors.border }]}>
      ...
      <View style={styles.topbarRight}>
        <Text style={[styles.controlTag, { color: colors.success }]}>DUAL CONTROL</Text>
    ...
    <View style={styles.grid}>
      ...
      <View style={styles.sideColumn}>
        <Panel title="Shared signal controls" subtitle="Changes here immediately affect Telegram and the next configured closed-candle cycle." colors={colors}>
          {config ? <>
            <View style={[styles.syncBanner, { backgroundColor: colors.background, borderColor: colors.border }]}><View><Text style={[styles.syncTitle, { color: colors.foreground }]}>Config v{config.configVersion}</Text><Text style={[styles.syncCopy, { color: colors.muted }]}>Last change: {config.lastChangedBy.toLowerCase()}</Text></View><Text style={[styles.syncState, { color: config.isPaused ? colors.warning : colors.success }]}>{config.isPaused ? "PAUSED" : "ACTIVE"}</Text></View>
            ...
            <ControlGroup label={`Alert threshold · ${Math.round(config.alertThreshold * 100)}%`} help="Minimum normalized evidence score required before a Telegram alert is attempted."><ThresholdRail value={config.alertThreshold} onChange={(alertThreshold) => thresholdMutation.mutate({ alertThreshold })} colors={colors} disabled={isSaving} /></ControlGroup>
