...
type Workspace = "research" | "live" | "controls";
...
function AuthenticatedDashboard({ username, onSignOut }: { username: string; onSignOut: () => Promise<void> }) {
  ...
  const liveQuote = trpc.market.liveQuote.useQuery({ assetSymbol }, { enabled: false, retry: false });
  ...
  const publicRefreshMutation = trpc.bot.refreshPublicData.useMutation({
    onSuccess: async (result) => {
      await Promise.all([chart.refetch(), latest.refetch(), refreshSharedState()]);
      setControlNotice({ tone: "success", text: result.paused ? "Signal processing is paused." : t("refreshCompleted", { cycles: result.cycleCount }) });
    },
    onError: () => setControlNotice({ tone: "error", text: t("refreshFailed") }),
  });
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]}><ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}><View style={styles.shell}>
    <View style={[styles.topbar, { borderColor: colors.border }]}>
      <View><Text style={[styles.brand, { color: colors.primary }]}>CRYPTO SIGNAL</Text><Text style={[styles.brandSub, { color: colors.muted }]}>{t("signedInAs", { username })}</Text></View>
      <View style={styles.topbarRight}>
        <Text style={[styles.controlTag, { color: colors.success }]}>{t("webControl")}</Text>
        <View style={localizedStyles.languageChoices}><Pressable accessibilityLabel={t("languageEnglish")} onPress={() => setLocale("en")} style={({ pressed }) => [localizedStyles.languageChoice, { borderColor: locale === "en" ? colors.primary : colors.border, backgroundColor: locale === "en" ? `${colors.primary}14` : colors.background }, pressed && styles.pressed]}><Text style={[localizedStyles.languageChoiceText, { color: locale === "en" ? colors.primary : colors.muted }]}>EN</Text></Pressable><Pressable accessibilityLabel={t("languageVietnamese")} onPress={() => setLocale("vi")} style={({ pressed }) => [localizedStyles.languageChoice, { borderColor: locale === "vi" ? colors.primary : colors.border, backgroundColor: locale === "vi" ? `${colors.primary}14` : colors.background }, pressed && styles.pressed]}><Text style={[localizedStyles.languageChoiceText, { color: locale === "vi" ? colors.primary : colors.muted }]}>VI</Text></Pressable></View>
    ...
    <View style={styles.hero}><Text style={[styles.eyebrow, { color: colors.primary }]}>{t("researchEyebrow")}</Text><Text style={[styles.title, { color: colors.foreground }]}>{t("researchTitle")}</Text><Text style={[styles.subtitle, { color: colors.muted }]}>{t("researchSubtitle")}</Text></View>
    ...
    <WorkspaceSwitcher value={workspace} onChange={setWorkspace} colors={colors} />
    <View style={styles.grid}>
      <View style={styles.primaryColumn}>
        {workspace === "live" ? <><LiveMarketPanel snapshot={liveSnapshot.data} health={liveHealth.data ?? []} observation={liveObservations.data?.[0] ?? null} quote={liveQuote.data} quoteLoading={liveQuote.isFetching} quoteError={liveQuote.error?.message ?? null} onRefreshQuote={() => void liveQuote.refetch()} /><OperationalAuditPanel runnerHealth={status.data?.runnerHealth ?? null} /></> : null}
        {workspace === "research" ? <><Panel title={t("historicalEvidence")} subtitle={t("historicalEvidenceSubtitle")} colors={colors}>
          ...
          <View style={[localizedStyles.publicRefresh, { backgroundColor: colors.background, borderColor: colors.border }]}><View style={localizedStyles.publicRefreshCopy}><Text style={[localizedStyles.publicRefreshTitle, { color: colors.foreground }]}>{t("refreshLatest")}</Text><Text style={[localizedStyles.publicRefreshHelp, { color: colors.muted }]}>{t("refreshLatestHelp")}</Text></View><Pressable disabled={publicRefreshMutation.isPending || status.data?.isPaused} onPress={() => publicRefreshMutation.mutate({ assetSymbol, timeframe })} style={({ pressed }) => [localizedStyles.publicRefreshButton, { backgroundColor: colors.primary }, pressed && styles.pressed, (publicRefreshMutation.isPending || status.data?.isPaused) && styles.disabled]}>{publicRefreshMutation.isPending ? <ActivityIndicator color="#FFFFFF" /> : <Text style={localizedStyles.publicRefreshButtonText}>{t("refreshNow")}</Text>}</Pressable></View>
          {publicRefreshMutation.isPending ? <Text style={[styles.savingText, { color: colors.muted }]}>{t("refreshInProgress")}</Text> : null}
          {chart.isLoading ? <ActivityIndicator color={colors.primary} /> : chart.data?.candles.length ? <PriceHistoryChart candles={chart.data.candles} signals={chart.data.signals} assetSymbol={assetSymbol} timeframe={timeframe} annotations={chart.data.annotations} /> : <Empty title="No chart history" detail="Run the configured closed-candle cycle to populate this pair and timeframe." colors={colors} />}
        </Panel>
        ...
        <Panel title="Latest confirmed signal" subtitle="Closed-candle research evidence. It remains separate from live observations." colors={colors}>{latest.data ? <SignalCard signal={latest.data} /> : <Empty title="No signal snapshot" detail="Refresh public candle research to calculate the currently selected market." colors={colors} />}</Panel></> : null}
      </View>
      <View style={styles.sideColumn}>
        ...
        <Panel title="Signal controls" subtitle="Changes here affect the web research workflow and the next configured closed-candle cycle. Telegram shares them only when enabled." colors={colors}>
          {config ? <>
            ...
            <ControlGroup label={`Methodology rules · ${config.enabledMethodologies.length}/${METHODOLOGY_RULES.length}`} help="Each rule must also have its parent family enabled. Wyckoff, SMC, and Elliott entries are explicit closed-candle research proxies—not trade instructions.">
              {RULE_FAMILIES.map((family) => { const rules = METHODOLOGY_RULES.filter((rule) => rule.family === family.id); return rules.length ? <View key={family.id} style={styles.ruleSubgroup}><Text style={[styles.ruleSubgroupTitle, { color: colors.muted }]}>{family.label}</Text><ToggleGrid items={rules.map((rule) => rule.id)} labels={Object.fromEntries(rules.map((rule) => [rule.id, rule.label]))} values={config.enabledMethodologies} onToggle={(value) => { const next = toggleConfigValue(config.enabledMethodologies, value); if (next.length) methodologiesMutation.mutate({ enabledMethodologies: next }); }} colors={colors} disabled={isSaving} /></View> : null; })}
            </ControlGroup>
...
function WorkspaceSwitcher({ value, onChange, colors }: { value: Workspace; onChange: (next: Workspace) => void; colors: ReturnType<typeof useColors> }) { const { t } = useI18n(); const items: Array<{ id: Workspace; label: string; detail: string }> = [{ id: "research", label: t("workspaceResearch"), detail: t("workspaceResearchDetail") }, { id: "live", label: t("workspaceLive"), detail: t("workspaceLiveDetail") }, { id: "controls", label: t("workspaceControls"), detail: t("workspaceControlsDetail") }]; return <View style={[styles.workspaceSwitcher, { backgroundColor: colors.surface, borderColor: colors.border }]}>{items.map((item) => <Pressable key={item.id} onPress={() => onChange(item.id)} style={({ pressed }) => [styles.workspaceOption, { borderColor: value === item.id ? colors.primary : "transparent", backgroundColor: value === item.id ? `${colors.primary}14` : "transparent" }, pressed && styles.pressed]}><Text style={[styles.workspaceLabel, { color: value === item.id ? colors.foreground : colors.muted }]}>{item.label}</Text><Text style={[styles.workspaceDetail, { color: value === item.id ? colors.primary : colors.muted }]}>{item.detail}</Text></Pressable>)}</View>; }
