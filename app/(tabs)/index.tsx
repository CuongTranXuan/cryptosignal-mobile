...
function AuthenticatedDashboard({ username, onSignOut }: { username: string; onSignOut: () => Promise<void> }) {
  ...
  const pauseMutation = trpc.bot.controls.setPaused.useMutation({ onSuccess: () => confirmControl("Signal processing state updated for Telegram and dashboard."), onError: reportControlError });
  ...
  const timeframesMutation = trpc.bot.controls.setTimeframes.useMutation({ onSuccess: () => confirmControl("Timeframe selection updated for Telegram and dashboard."), onError: reportControlError });
