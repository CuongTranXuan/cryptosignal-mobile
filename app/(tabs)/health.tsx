    ...
    <ScreenContainer className="px-4" edges={["top", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {status.data ? <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <HealthRow label="Telegram poller" value={status.data.telegramPoller.state.replaceAll("_", " ")} valueColor={status.data.telegramPoller.state === "RUNNING" ? colors.success : status.data.telegramPoller.state === "CONFLICT" || status.data.telegramPoller.state === "DEGRADED" ? colors.warning : colors.muted} colors={colors} />
        ...
        {status.data?.telegramPoller.state === "CONFLICT" ? <View style={[styles.notice, { backgroundColor: `${colors.warning}14` }]}><Text style={[styles.noticeTitle, { color: colors.warning }]}>Another poller is using this bot token</Text><Text style={[styles.noticeCopy, { color: colors.foreground }]}>Telegram permits one active getUpdates consumer. Stop the duplicate runtime or set TELEGRAM_POLLING_ENABLED=false on dashboard-only instances, then keep one persistent poller enabled.</Text></View> : null}
