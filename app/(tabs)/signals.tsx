    ...
    <ScreenContainer className="px-4" edges={["top", "left", "right"]}>
      <FlatList
        ...
        ListHeaderComponent={<View style={styles.header}>
          ...
          {chart.isLoading ? <ActivityIndicator color={colors.primary} /> : chart.data?.candles.length ? <PriceHistoryChart candles={chart.data.candles} signals={chart.data.signals} assetSymbol={assetSymbol} timeframe={timeframe} annotations={chart.data.annotations} /> : <Empty title="No candle history yet" detail="Run the configured closed-candle cycle to backfill chart history for this pair and timeframe." colors={colors} />}
