...
def build_snapshot(symbol: str, timeframe: str, dataframe: pd.DataFrame, config_version: int = 1) -> dict:
    ...
    finding_specs = [
        ("TREND", "EMA_TREND_V1", abs(float(row["signal_score"])) >= 0.3),
        ("MOMENTUM", "RSI_MACD_CONFIRMATION_V1", abs(float(row["signal_score"])) >= 0.2),
        ("CANDLE_PATTERN", "BULLISH_ENGULFING_V1", float(row["bullish_engulfing_raw"]) > 0),
        ("CANDLE_PATTERN", "BEARISH_ENGULFING_V1", float(row["bearish_engulfing_raw"]) > 0),
        ("CANDLE_PATTERN", "DOJI_V1", float(row["doji_raw"]) > 0),
    ]
    findings = [
        {
        ...
        for family, rule_id, active in finding_specs
        if active
    ]
...
def main() -> None:
    ...
    snapshot = build_snapshot(args.symbol.upper(), args.timeframe, load_closed_candles(args.symbol.upper(), args.timeframe, args.limit), args.config_version)
    ...
    response.raise_for_status()
    print(json.dumps({"submission": response.json()}, indent=2, sort_keys=True))
