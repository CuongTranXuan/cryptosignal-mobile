...
- Task 8: fix round 1/5 (independent review tightened the replay end boundary, isolated the real ClickHouse adapter, added bounded Redis connection safeguards, sanitized health summaries, and made replay-degradation persistence best-effort)
...
- Task 11: fix round 1/5 (independent review hardened public MCP request identifiers and response-preview serialization; no dashboard route invokes discovery automatically)
- Task 11: complete (denylist-first, disabled-by-default public MCP client, confirmation-gated dashboard route, isolated optional profile, focused security tests, TypeScript, lint, and production adapter bundle checks pass; deterministic serial regression passed with 90 tests and 1 skipped while the external Telegram credential reachability test timed out against api.telegram.org)
- Task 12: fix round 1/5 (independent review added actual manifest-ledger export and optional read-only SeaweedFS HeadObject checksum verification while retaining local staged verification)
- Task 12: complete (non-destructive backup, explicit restore staging, read-only archive verification, profile-aware deployment checks, capacity evidence template, and operations documentation pass focused tests, bash syntax, deterministic serial regression, TypeScript, lint, and production bundle checks; Docker Compose rendering remains an external host acceptance gate because Docker is unavailable in this sandbox)
- Documentation consolidation: complete (replaced stale overlapping design and implementation documents with a current architecture guide and compact documentation map; retained operations, research-source, capacity, and implementation-plan records)
- Final integration validation: deterministic suite passed with 92 tests and 1 skipped, along with TypeScript, lint, shell syntax, and all production server artifacts. The external Telegram `getMe` credential check twice timed out reaching `api.telegram.org`, and Docker Compose rendering remains unrun because Docker is not installed in this sandbox; both are host-side acceptance checks rather than code regressions.
